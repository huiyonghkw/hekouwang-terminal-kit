...
## Docs · README 瘦身（无版本号变更）

- 仓根 `README.md` / `README.zh-CN.md` 收成快速开始（约 130 行）：装 → 得到什么 → 一张对比图说清分档 → 出问题
- 原长文迁到 `docs/manual.md` / `docs/manual.zh-CN.md`，进阶、FAQ、设计原则都在手册里
- 付费能力仍然只在分档表与购买页出现；付费实现（Skill / 品牌色板 / 多终端生成器等）不进本公开仓
